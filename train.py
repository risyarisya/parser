...
def sample_x_and_label_from_data_distribution(batchsize):
    x_batch = np.zeros((batchsize, 2), dtype=np.float32)
    label_index_batch = np.zeros((batchsize, 1), dtype=np.int32)
    label_one_hot = np.zeros((batchsize, 2), dtype=np.float32)
    
    for j in range(batchsize):
        data_index = np.random.randint(len(dataset))
        x_batch[j] = dataset[data_index]
...
#	if config.use_gpu:
#		x_batch.to_gpu()
#		label_one_hot.to_#	return x_batch, label_index_batch, label_one_hot
